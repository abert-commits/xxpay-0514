...
@Service
public class BeibeilepayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(BeibeilepayPaymentService.class);
    ...
    public JSONObject doJuheRyPayReq(PayOrder payOrder, String pay_code) {
        ...
        try {
            ...
            map.put("success_url", payConfig.getReturnUrl(channelName)); //页面跳转返回地址
            map.put("fail_url", payConfig.getReturnUrl(channelName)); //页面跳转返回地址
            map.put("type", channelPayConfig.getChannelId()); //银行编码
            map.put("timestamp", String.valueOf(System.currentTimeMillis() / 1000)); //请求时间
            map.put("method", "html");//商品名称
            ...
            String res = (String) XXPayUtil.buildRequestHtml(map,channelPayConfig.getReqUrl() + "/pay/transaction/payment");
    ...
    public JSONObject query(PayOrder payOrder) {
        ...
        try {
            ...
            if (code.equals("00")) {
                ...
                System.out.println("贝贝乐------》》》》》》《《《《《《++++++" + map1.toString());
