...
@Service
public class LeitingPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(LeitingPaymentService.class);
    ...
    public JSONObject doHuaFeiPayReq(PayOrder payOrder, String channel) {
        ...
        try {
            ...
            map.put("notify_url", URLEncoder.encode(payConfig.getNotifyUrl(getChannelName()),"UTF-8").toUpperCase()); //支付结果后台回调URL
            ...
            if ("100".equals(resultCode)) {
                String payJumpUrl = resObj.getJSONObject("data").getString("orderStr");
