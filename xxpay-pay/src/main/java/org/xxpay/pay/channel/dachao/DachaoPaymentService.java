...
@Service
public class DachaoPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(DachaoPaymentService.class);
    ...
    public JSONObject doHuaFeiPayReq(PayOrder payOrder, String channel) {
        ...
        try {
            ...
            map.put("noticeUrl", payConfig.getNotifyUrl(channelName)); //支付结果后台回调URL
            String sign = PayDigestUtil.md5(map.get("appId") +"^"+
                    map.get("orderNo") +"^"+ map.get("money") +"^"+
                    map.get("timestamp")  +"^"+
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/xunpay-alipay-api/order/create", sendMsg);
            _log.info(logPrefix + "******************上游返回数据:{}", res);

            JSONObject resObj = JSONObject.parseObject(res);
            String code = resObj.getString("result");
    ...
    public JSONObject query(PayOrder payOrder) {
        ...
        try {
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/xunpay-alipay-api/order/query", sendMsg);
            _log.info("上游返回信息：" + res);

            JSONObject resObj = JSONObject.parseObject(res);
            String code = resObj.getString("result");
            String status = resObj.getJSONObject("data").getString("status");
            if (code.equals("success")) {
                retObj.put("status", "2");
            } else {
                retObj.put("status", "1");
            }
            retObj.put("msg", "响应Code:" + code + ",status:" + status + "");
