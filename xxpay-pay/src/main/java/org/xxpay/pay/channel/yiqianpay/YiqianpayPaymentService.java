...
@Service
public class YiqianpayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(YiqianpayPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY:
                retObj = doYiqianRyPayReq(payOrder, "HAliPayH5"); //支付宝话费H5
                break;
            default:
    ...
    public JSONObject doYiqianRyPayReq(PayOrder payOrder, String pay_code) {
        String logPrefix = "【昊乾支付统一下单】";
        ...
        try {
            ...
            map.put("trade_type", pay_code); //支付类型
            ...
            map.put("back_url", payConfig.getReturnUrl(channelName)); //页面跳转返回地址
            map.put("mch_create_ip", "127.0.0.1"); //回调Ip
            map.put("attach","你好");//attach备注异步通知会带回
            ...
            String res = (String) XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/pay/index",sendMsg);
            _log.info(logPrefix + "******************上游返回数据:{}", res);
            JSONObject resObj = JSONObject.parseObject(res);
            if (resObj.getString("code").equals("0")) {
                String payDayuUrl = resObj.getString("message");
    ...
    public JSONObject query(PayOrder payOrder) {
        ...
        try {
            ...
            String res = (String) XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/pay/query",sendMsg);
            _log.info("上游返回信息：" + res);
            JSONObject resObj = JSONObject.parseObject(res);
            String code = resObj.getString("state");
            if (code.equals("00")) {
                ...
                System.out.println("昊乾------》》》》》》《《《《《《++++++" + map1.toString());
