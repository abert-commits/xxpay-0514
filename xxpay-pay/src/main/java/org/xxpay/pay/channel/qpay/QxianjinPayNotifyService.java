...
@Service
public class QxianjinPayNotifyService extends BasePayNotify {

    private static final MyLog _log = MyLog.getLog(QxianjinPayNotifyService.class);
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理Q支付现金红包回调】";
        _log.info("====== 开始处理Q支付现金红包回调通知 ======");
...
//            JSONObject params =JSONObject.parseObject(GetInput(notifyData));
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) throws Exception {
        ...
        // 校验结果是否成功
        String orderNo = params.getString("out_trade_no"); // 商户唯一订单号
        ...
        //
        String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/pay/Query/" + map.get("order_no"), "");
        JSONObject resObj = JSONObject.parseObject(res);
        String retStatus = resObj.getString("status");
        String retCode = resObj.getString("code");
        String mchOrderNo = resObj.getString("mch_order_no");
        String checkMchOrderNo = PayDigestUtil.md5(map.get("out_trade_no").toString(), "UTF-8").toUpperCase();

        if (!"2".equals(retCode) && !mchOrderNo.equals(checkMchOrderNo)) {
            _log.error("状态匹配失败. payOrderId={}, ", orderNo);
            payContext.put("retMsg", "状态匹配失败");
            return false;
        }
