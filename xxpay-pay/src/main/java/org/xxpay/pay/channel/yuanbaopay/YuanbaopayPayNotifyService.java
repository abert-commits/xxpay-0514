package org.xxpay.pay.channel.yuanbaopay;
...
@Service
public class YuanbaopayPayNotifyService extends BasePayNotify {

    private static final MyLog _log = MyLog.getLog(YuanbaopayPayNotifyService.class);

    @Override
    public String getChannelName() {
        return PayConstant.CHANNEL_NAME_YUANBAO;
    }

    @Override
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理元宝支付回调】";
        _log.info("====== 开始处理元宝支付回调通知 ======");
        ...
        try {
            ...
            params.put("bankCode", request.getParameter("bankCode"));
            params.put("outOrderNo", request.getParameter("outOrderNo"));
//            params.put("extend", request.getParameter("extend"));
            params.put("responseMsg", request.getParameter("responseMsg"));
            params.put("goodsName", request.getParameter("goodsName"));
            params.put("mchId", request.getParameter("mchId"));
            params.put("responseState", request.getParameter("responseState"));
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) {
        ...
        map.put("amount", amount);// 单位 元
        map.put("bankCode", params.getString("bankCode"));
        map.put("outOrderNo", params.getString("outOrderNo"));
        map.put("responseMsg", params.getString("responseMsg"));
        map.put("goodsName", params.getString("goodsName"));
        map.put("mchId", params.getString("mchId"));
        map.put("responseState", params.getString("responseState"));
        map.put("orderNo", params.getString("orderNo"));
        map.put("payTime", params.getString("payTime"));
        map.put("signType", params.getString("signType"));

        String signStr = XXPayUtil.mapToString(map).replace(">", "");
        signStr=signStr + channelPayConfig.getmD5Key();
        String signValue = PayDigestUtil.md5(signStr,"utf-8").toLowerCase();
