...
@Service
public class YuzhouPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(YuzhouPaymentService.class);
    ...
    public JSONObject doMayiPayReq(PayOrder payOrder, String channel) {
        ...
        try {
            ...
            map.put("notifyurl", URLDecoder.decode(payConfig.getNotifyUrl(channelName), "UTF-8"));
            if (!channelId.contains("SDK")) {
                map.put("appraw", "0");//0 = 返回支付宝H5支付链接,1 = 返回支付宝SDK支付字符串（针对APP端唤起支付，不支持网页支付）
            }else {
                map.put("appraw", "1");//0 = 返回支付宝H5支付链接,1 = 返回支付宝SDK支付字符串（针对APP端唤起支付，不支持网页支付）
            }
