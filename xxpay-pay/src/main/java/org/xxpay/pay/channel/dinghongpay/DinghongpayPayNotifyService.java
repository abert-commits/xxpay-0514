package org.xxpay.pay.channel.dinghongpay;
...
@Service
public class DinghongpayPayNotifyService extends BasePayNotify {
    @Override
    public String getChannelName() {
        return PayConstant.CHANNEL_NAME_DINGH;
    }

    private static final MyLog _log = MyLog.getLog(DinghongpayPaymentService.class);

    @Override
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理鼎红支付回调】";
        _log.info("====== 开始处理鼎红支付回调通知 ======");
...
//        JSONObject jsonParam = PayDigestUtil.getJSONParam(request);
        ...
        try {
            ...
            params.put("sys_order_no", request.getParameter("sys_order_no"));
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) throws Exception {
        ...
        String sys_order_no = params.getString("sys_order_no"); // 平台流水号
        String outOrderNo = params.getString("out_order_no"); // 商户订单号
        String orderTime = params.getString("order_time"); //订单请求时间
        String cuid = params.getString("cuid"); //商户的用户ID

        BigDecimal c = new BigDecimal(params.getString("realPrice"));
        String realPrice = c.setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        ...
        signMap.put("merchant", merchant);
        signMap.put("amount", amount);
        signMap.put("sys_order_no", sys_order_no);
        signMap.put("out_order_no", outOrderNo);
        signMap.put("order_time", orderTime);
        signMap.put("realPrice", realPrice);
        signMap.put("attach", attach);
        signMap.put("cuid", cuid);
        ...
        long resAmount = new BigDecimal(realPrice).multiply(new BigDecimal(100)).longValue();
