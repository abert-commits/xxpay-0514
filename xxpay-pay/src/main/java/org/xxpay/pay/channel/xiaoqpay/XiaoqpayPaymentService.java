...
@Service
public class XiaoqpayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(XiaoqpayPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY:
                retObj = doHuaFeiPayReq(payOrder, "ZFBH5"); //支付宝转账H5
                break;
            default:
...
//            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/pay",sendMsg);
            String payUrl = XXPayUtil.buildRequestHtml(map, channelPayConfig.getReqUrl() + "/pay");
            _log.info(logPrefix + "******************上游返回数据:{}", payUrl);
