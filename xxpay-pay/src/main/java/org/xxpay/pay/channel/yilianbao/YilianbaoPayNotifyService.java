package org.xxpay.pay.channel.yilianbao;
...
@Service
public class YilianbaoPayNotifyService extends BasePayNotify {

    private static final MyLog _log = MyLog.getLog(YilianbaoPayNotifyService.class);
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理易联宝回调】";
        _log.info("====== 开始处理易联宝回调通知 ======");
...
//        JSONObject jsonParam = PayDigestUtil.getJSONParam(request);
        ...
        try {
            ...
            params.put("merchantsNo", request.getParameter("merchantsNo"));
            ...
            params.put("referTime", request.getParameter("referTime"));
            ...
            if ("10000".equals(params.getString("status"))) {
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) throws Exception {
        ...
        // 校验结果是否成功
        String merchantsNo = params.getString("merchantsNo");  //商户号
        ...
        String transactionNo = params.getString("transactionNo"); //交易流水号
        String referTime = params.getString("referTime"); //交易时间
        ...
        signMap.put("merchantsNo", merchantsNo);
        signMap.put("amount", amount);
        signMap.put("orderNo", orderNo);
        signMap.put("transactionNo", transactionNo);
        signMap.put("referTime", referTime);
        signMap.put("status", status);
        String sginString1 = XXPayUtil.mapToString(signMap).replace(">", "");

        boolean verify = RSASHA256withRSAUtils.verify(sginString1.getBytes(), channelPayConfig.getRsaPublicKey(), sign);
        if (!verify) {
