...
@Service
public class ShunfupayPayNotifyService extends BasePayNotify {
    @Override
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理瞬付支付回调】";
        _log.info("====== 开始处理瞬付支付回调通知 ======");
        ...
        try {
            ...
            params.put("member_id", jsonParam.getString("member_id"));
            params.put("order_no", jsonParam.getString("order_no"));
            params.put("money", jsonParam.getString("money"));
            params.put("status", jsonParam.getString("status"));
            params.put("sdpayno", jsonParam.getString("sdpayno"));
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) throws Exception {
        ...
        String memberid = params.getString("member_id");  //商户号
        String amount = params.getString("money"); // 交易金额
        ...
        String sdpayno = params.getString("sdpayno"); //交易流水号
        ...
        String sign1 = MessageFormat.format("member_id={0}&status={1}&sdpayno={2}&order_no={3}&money={4}&paytype={5}&{6}", memberid
                , status, sdpayno, outOrderNo, amount, paytype, channelPayConfig.getmD5Key());
        System.out.println("回调签名《《《》》》》》++++" + sign1);
        // md5 加密
        String md5 = PayDigestUtil.md5(sign1, "utf-8").toLowerCase();
        if (!md5.equals(sign)) {
