package org.xxpay.pay.channel.feifan;
...
@Service
public class FeifanPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(FeifanPaymentService.class);

    @Override
    public String getChannelName() {
        return PayConstant.CHANNEL_NAME_FEIFAN;
    }
    ...
    public JSONObject doHuaFeiPayReq(PayOrder payOrder, String channel) {
        String logPrefix = "【非凡闲鱼统一下单】";
        JSONObject payInfo = new JSONObject();
        try {
            ChannelPayConfig channelPayConfig = new ChannelPayConfig(getPayParam(payOrder));
            SortedMap map = new TreeMap();
            map.put("return_type", "sdk");
            map.put("appid", channelPayConfig.getMchId()); //应用ID
            ...
            map.put("success_url", payConfig.getReturnUrl(getChannelName()));
            map.put("error_url", payConfig.getReturnUrl(getChannelName()));
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/payment-gateway/gateway/api/unifiedorder.do?format=json", sendMsg, "utf-8", "application/x-www-form-urlencoded");
