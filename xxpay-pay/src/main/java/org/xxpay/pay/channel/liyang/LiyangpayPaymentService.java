...
@Service
public class LiyangpayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(LiyangpayPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY_PDDSDK:
                retObj = doLiYangRyPayReq(payOrder, "H5ZFB"); //支付宝拼多多SDK
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAYQR:
                retObj = doLiYangRyPayReq(payOrder, "H5ZFB"); //支付宝扫码
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_WXPAYZXQR:
                retObj = doLiYangRyPayReq(payOrder, "WXZF"); //微信扫码
                break;
            default:
    ...
    public JSONObject doLiYangRyPayReq(PayOrder payOrder, String pay_code) {
        String logPrefix = "【黎阳支付统一下单】";
        ...
        try {
            ...
            map.put("totalAmount", String.valueOf(amount));// 金额
            map.put("corp_flow_no", payOrder.getPayOrderId());// 商户订单号
            map.put("pay", "pay");// pay
            System.out.println(map.toString());
            String sign =PayDigestUtil.md5(channelPayConfig.getMchId()+"pay"+amount+payOrder.getPayOrderId()+"3AD23CD1823CAD8B60D568052D85430720200417221653","UTF-8");
           // String sign = PayDigestUtil.getSign(map, channelPayConfig.getmD5Key());
            map.put("notify_url", payConfig.getNotifyUrl(channelName)); //异步通知地址
//            map.put("client_ip", payConfig.getReturnUrl(channelName));// 同步跳转地址
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/trade/doPay.do", sendMsg);
            _log.info(logPrefix + "******************上游返回数据:{}", res);
            JSONObject resObj = JSONObject.parseObject(res);
            if (resObj.getString("Code").equals("00")) {
                String payUrl = resObj.getString("Msg");
    ...
    public JSONObject query(PayOrder payOrder) {
        String logPrefix = "【黎阳支付订单查询】";
        JSONObject payInfo = new JSONObject();
        JSONObject retObj = new JSONObject();
        try {
            ChannelPayConfig channelPayConfig = new ChannelPayConfig(getPayParam(payOrder));
            SortedMap map = new TreeMap();
            map.put("reqMsgId", payOrder.getPayOrderId());// 订单号
            map.put("merchantId", channelPayConfig.getMchId()); // 商户号
            map.put("corp_flow_no", ""); // 平台流水号
            String sign = PayDigestUtil.md5(channelPayConfig.getMchId()+payOrder.getPayOrderId()+""+"3AD23CD1823CAD8B60D568052D85430720200417221653","UTF-8");
            _log.info(logPrefix + "******************sign:{}", sign);
            map.put("sign", sign);
            String sendMsg = XXPayUtil.mapToString(map).replace(">", "");
            JSONObject jsonObj = new JSONObject(map);
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/trade/quickQuery.do", sendMsg);
            ...
            if (code.equals("00")) {
                ...
                System.out.println("黎阳------》》》》》》《《《《《《++++++" + map1.toString());
