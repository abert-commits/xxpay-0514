...
@Service
public class XuezhifuPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(XuezhifuPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            ...
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_WXPAY:
                retObj = doMayiPayReq(payOrder, "4");
                break;
    ...
    public JSONObject doMayiPayReq(PayOrder payOrder, String channel) {
        ...
        try {
            ...
            map.put("cash", AmountUtil.convertCent2Dollar(payOrder.getAmount().toString()));// 单位 元
            map.put("server_url", payConfig.getNotifyUrl(channelName));//异步通知地址
            map.put("brower_url", "127.0.0.1");//同步回调地址
            map.put("order_id", payOrder.getPayOrderId());//订单号
            map.put("order_time", DateUtil.getRevTime());
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/api/recharge/index", sendMsg);
