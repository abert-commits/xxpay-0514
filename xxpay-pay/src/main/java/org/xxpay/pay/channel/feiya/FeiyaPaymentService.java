package org.xxpay.pay.channel.feiya;
...
@Service
public class FeiyaPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(FeiyaPaymentService.class);

    @Override
    public String getChannelName() {
        return PayConstant.CHANNEL_NAME_FEIYA;
    }
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY_PDDSDK:
                retObj = doJuheRyPayReq(payOrder, "1076"); //支付宝拼多多
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_WXPAY_PDD:
                retObj = doJuheRyPayReq(payOrder, "1075");
                break;
            default:
    ...
    public JSONObject doJuheRyPayReq(PayOrder payOrder, String pay_code) {
        ...
        try {
            ...
            String res = (String) XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/paididx/index.html",sendMsg);
            _log.info(logPrefix + "******************上游返回数据:{}", res);
            JSONObject resObj = JSONObject.parseObject(res);
            if (resObj.getString("status") != null && resObj.getString("status").equals("error")) {
                payInfo.put("errDes", "下单失败[" + resObj.getString("msg") + "]");
                payInfo.put(PayConstant.RETURN_PARAM_RETCODE, PayConstant.RETURN_VALUE_FAIL);
                return payInfo;
            } else if (resObj.getString("sdk") != null) {
                JSONObject payParams = new JSONObject();
                String payDayuUrl = resObj.getString("sdk");
...
//                String payDayuUrl = jsonObject1.getString("mweb_url");
//                JSONObject payParams = new JSONObject();
//                payParams.put("payJumpUrl", payDayuUrl);
//                if (payOrder.getChannelId().contains("SDK")) {
//                    if (payDayuUrl.contains("form")) {
...
//            }
    ...
    public JSONObject query(PayOrder payOrder) {
        ...
        try {
            ...
            String res = (String) XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/paidtrd/query.html",sendMsg);
            ...
            if (code.equals("00")) {
                ...
                System.out.println("飞亚------》》》》》》《《《《《《++++++" + map1.toString());
