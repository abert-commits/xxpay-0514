...
@Service
public class HuichaoPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(HuichaoPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY:
                retObj = doHuaFeiPayReq(payOrder, "bbwturlpay"); //pay.alipay.h5 支付宝H5
                break;
            default:
    ...
    public JSONObject doHuaFeiPayReq(PayOrder payOrder, String channel) {
        ...
        try {
            ...
            map.put("return_url", payConfig.getReturnUrl(getChannelName())); //支付结果URL

            String signMsg = XXPayUtil.mapToString(map).replace(">", "");
            String sign = PayDigestUtil.md5((signMsg + "&token=" + channelPayConfig.getmD5Key()).toLowerCase(), "utf-8");
            _log.info(logPrefix + "******************sign:{}", sign);
            map.put("sign", sign); //签名

            map.put("username", "韩信支付"); //商品主题
            map.put("user_id", "123"); //会员id
            map.put("pay_type", channel); //支付产品ID
            map.put("version", "v1"); //版本号
            String sendMsg = XXPayUtil.mapToString(map).replace(">", "");
            _log.info(logPrefix + "******************sendMsg:{}", sendMsg);
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/order/order", sendMsg);
