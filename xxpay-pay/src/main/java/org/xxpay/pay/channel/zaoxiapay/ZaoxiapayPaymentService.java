...
@Service
public class ZaoxiapayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(ZaoxiapayPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_WXPAYZXQR:
                retObj = doZaoxiaPayReq(payOrder, "/v1/api/order","2");//微信扫码
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_WXPAY:
                retObj = doZaoxiaPayReq(payOrder, "/v1/api/order","3"); //微信H5
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAYQR:
                retObj = doZaoxiaPayReq(payOrder, "/v1/api/order","1"); //支付宝扫码
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY:
                retObj = doZaoxiaPayReq(payOrder, "/v1/api/order","4"); // 支付宝H5
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY_SDK:
                retObj = doZaoxiaPayReq(payOrder, "/v1/api/order","5"); //支付宝SDK
                break;
            default:
    ...
    public JSONObject doZaoxiaPayReq(PayOrder payOrder, String url, String channel) {
        String logPrefix = "【造虾支付统一下单】";
...
//            }
            ...
            jsonObject.put("access_token", accessToken.toString());// 商家号

            Map map = new HashMap();
            jsonObject.put("order_no", payOrder.getPayOrderId()); //商户唯一订单号
            jsonObject.put("money", payOrder.getAmount()); //商户订单金额
            if (StringUtils.isNotEmpty(channelPayConfig.getRsapassWord())) {
                channel = channelPayConfig.getRsapassWord();
            }
            jsonObject.put("type", channel); //付款类型
            jsonObject.put("body", "韩信支付"); //韩信支付
            jsonObject.put("detail", "韩信支付"); //商品详情
            jsonObject.put("notify_url", payConfig.getNotifyUrl(getChannelName())); //异步通知地址
            if ("/v1/api/order".equals(url)) {
                jsonObject.put("client_ip", "127.0.0.1"); //客户端IP
            }
            jsonObject.put("product_id", "65"); //商品ID
//            jsonObject.put("param", map);// 商家号
            ...
            if (success) {
                    String payJumpUrl = resObj.getJSONObject("value").getString("code_url");
    ...
    private boolean getAccessToken(ChannelPayConfig channelPayConfig) {
        String logPrefix = "【造虾支付订单获取access_token】";
        ...
        String res = XXPayUtil.postJson(channelPayConfig.getReqUrl() + "/v1/api/getAccessToken", map.toJSONString());
        _log.info("上游返回信息：" + res);
        JSONObject resObj = JSONObject.parseObject(res);
        boolean success = resObj.getBoolean("success");
        if (success) {
            accessToken=new StringBuffer();
//            expireTime = resObj.getJSONObject("value").getInteger("expire_time") - 200;
            accessToken.append(resObj.getJSONObject("value").getString("access_token"));
