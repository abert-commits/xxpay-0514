package org.xxpay.pay.channel.dacaishen;
...
@Service
public class DacaispayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(DacaispayPaymentService.class);

    @Override
    public String getChannelName() {
        return PayConstant.CHANNEL_NAME_DACAISHEN;
    }
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY_PDDSDK:
                retObj = doJuheRyPayReq(payOrder, "903"); //支付宝拼多多
                break;
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY:
                retObj = doJuheRyPayReq(payOrder, "903"); //支付宝H5
                break;
            default:
    ...
    public JSONObject doJuheRyPayReq(PayOrder payOrder, String pay_code) {
        ...
        try {
            ...
            String res = (String) XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/Pay/Index.ashx",sendMsg);
            _log.info(logPrefix + "******************上游返回数据:{}", res);
            JSONObject resObj = JSONObject.parseObject(res);
            if (resObj.getString("code").equals("1")) {
                JSONObject payParams = new JSONObject();
                String payJumpUrl = URLDecoder.decode(resObj.getString("sdkurl"),"UTF-8");
                payParams.put("payJumpUrl", payJumpUrl);
                if (payOrder.getChannelId().contains("SDK")) {
                        payParams.put("payMethod", PayConstant.PAY_METHOD_SDK_JUMP);
                } else {
                    if (payOrder.getChannelId().contains("ALIPAY")) {
                        String payJumpUrls = resObj.getString("sdkurl1");
    ...
    public JSONObject query(PayOrder payOrder) {
        ...
        try {
            ...
            if (code.equals("00")) {
                ...
                System.out.println("大财神------》》》》》》《《《《《《++++++" + map1.toString());
