...
@Service
public class YuanbPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(YuanbPaymentService.class);
    ...
    public JSONObject doYuanBaoPayReq(PayOrder payOrder, String channel) {
        ...
        try {
            ...
            map.put("goods_desc", "支付" + money + "元"); // 商品描述
            map.put("client_ip", payOrder.getClientIp()); // Ip地址
            map.put("format", "json"); // 接口返回格式
            map.put("notify_url", payConfig.getNotifyUrl(channelName)); // 异步回调地址
            map.put("time", String.valueOf((new Date().getTime())/1000)); // 时间戳
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/?c=Pay", sendMsg);
            ...
            if ("1".equals(retCode)) {
                String payJumpUrl = channelPayConfig.getReqUrl() + "/?c=Pay&a=info&osn="+retData.getString("order_sn");
                ...
                int result = rpcCommonService.rpcPayOrderService.updateStatus4Ing(payOrder.getPayOrderId(), retData.getString("order_sn"));
...
//            _log.info(logPrefix + "******************sendMsg:{}", sendMsg);
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/?c=Pay&a=query", sendMsg);
