...
@Service
public class WanxpayPayNotifyService extends BasePayNotify {
    @Override
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理万象支付回调】";
        _log.info("====== 开始处理万象支付回调通知 ======");
...
//        JSONObject jsonParam = PayDigestUtil.getJSONParam(request);
        ...
        try {
            ...
            params.put("merCode", request.getParameter("merCode"));
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) throws Exception {
        ...
        // 校验结果是否成功
        String merchant = params.getString("merCode");  //商户号
        String amount = params.getString("amount"); // 订单金额
        String outOrderNo = params.getString("orderNo"); // 商户订单号
        String transactionNo = params.getString("transactionNo"); //交易流水号
        String orderTime = params.getString("payTime"); //订单请求时间
        String returnCode = params.getString("returnCode"); //支付状态
        ...
        signMap.put("merCode", merchant);
        signMap.put("amount", amount);
        signMap.put("transactionNo", transactionNo);
        signMap.put("orderNo", outOrderNo);
        signMap.put("payTime", orderTime);
