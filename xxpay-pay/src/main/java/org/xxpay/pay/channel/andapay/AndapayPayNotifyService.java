...
import org.xxpay.core.common.util.AESUtils;
...
@Service
public class AndapayPayNotifyService extends BasePayNotify {
    @Override
    ...
    public JSONObject doNotify(Object notifyData) {
        String logPrefix = "【处理安达支付回调】";
        _log.info("====== 开始处理安达支付回调通知 ======");
...
//        JSONObject jsonParam = PayDigestUtil.getJSONParam(request);
    ...
    public boolean verifyPayParams(Map<String, Object> payContext) throws Exception {
        ...
        // 校验结果是否成功
        String content = params.getString("content");  //商户号
        ...
        String sys_order_no = params.getString("tx_no"); // 平台流水号
        String outOrderNo = params.getString("order_no"); // 商户订单号
        String status = params.getString("status"); //状态
        String extra = params.getString("extra"); //商户的用户ID

        BigDecimal c = new BigDecimal(params.getString("actual_amount"));
        String actual_amount = c.setScale(2, BigDecimal.ROUND_HALF_UP).toString();
        ...
        String contentStr = AESUtils.decryptBase64(content,channelPayConfig.getmD5Key());
        ...
        signMap.put("tx_no", sys_order_no);
        ...
        // md5 加密
        String jsonObject1 = JSONObject.toJSONString(signMap);
        String md5 =AESUtils.encryptBase64(jsonObject1,channelPayConfig.getmD5Key());
        if (!md5.equals(sign)) {
