package org.xxpay.pay.channel.xiaotaozi;
...
@Service
public class XiaotaoziPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(XiaotaoziPaymentService.class);

    @Override
    public String getChannelName() {
        return PayConstant.CHANNEL_NAME_XIAOTAOZI;
    }
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            ...
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY_SDK:
                retObj = doXiaoTaoZiPayReq(payOrder, ""); //pay.weixin.h5 微信H5
                break;
            default:
    ...
    public JSONObject doXiaoTaoZiPayReq(PayOrder payOrder, String channel) {
        String logPrefix = "【小桃子闲鱼统一下单】";
        JSONObject payInfo = new JSONObject();
        try {
            ChannelPayConfig channelPayConfig = new ChannelPayConfig(getPayParam(payOrder));
            SortedMap map = new TreeMap();
            map.put("mch_id", channelPayConfig.getMchId()); // 商户号
            map.put("trade_type", channelPayConfig.getChannelId()); // 支付方式
            ...
            map.put("mch_create_ip", payOrder.getClientIp()); // 用户端IP
            map.put("notify_url", payConfig.getNotifyUrl(getChannelName())); // 支付结果后台回调URL
            map.put("return_url", payConfig.getReturnUrl(getChannelName())); // 支付完成，或者取消后跳转地址
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/api", sendMsg, "utf-8");
    ...
    public JSONObject query(PayOrder payOrder) {
        String logPrefix = "【小桃子闲鱼订单查询】";
        JSONObject payInfo = new JSONObject();
        JSONObject retObj = new JSONObject();
        try {
            ChannelPayConfig channelPayConfig = new ChannelPayConfig(getPayParam(payOrder));
            SortedMap map = new TreeMap();
            map.put("mch_id", channelPayConfig.getMchId()); //应用ID
            ...
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/query", sendMsg);
