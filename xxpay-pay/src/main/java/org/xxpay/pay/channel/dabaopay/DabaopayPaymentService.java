...
@Service
public class DabaopayPaymentService extends BasePayment {

    private static final MyLog _log = MyLog.getLog(DabaopayPaymentService.class);
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY_PDDSDK:
                retObj = doDabaoRyPayReq(payOrder, "pdd"); //大宝宝拼多多SDK
                break;
            default:
    ...
    public JSONObject doDabaoRyPayReq(PayOrder payOrder, String pay_code) {
        ...
        try {
            ...
            String requestParam = XXPayUtil.mapToString(map).replace(">", "");
            String rsaprivateKey = channelPayConfig.getRsaprivateKey();
            String sign = RSASHA256withRSAUtils.sign(requestParam.getBytes(), rsaprivateKey);
            _log.info(logPrefix + "******************sign:{}", sign);
            map.put("sign", URLEncoder.encode(sign, "utf-8")); //签名

            String sendMsg = XXPayUtil.mapToString(map).replace(">", "");
            _log.info(logPrefix + "******************sendMsg:{}", sendMsg);
            String res = XXPayUtil.doPostQueryCmd(channelPayConfig.getReqUrl() + "/paybt.open/pdd/friendPay", sendMsg);
