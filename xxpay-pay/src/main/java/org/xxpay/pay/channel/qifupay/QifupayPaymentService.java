...
@Service
public class QifupayPaymentService extends BasePayment {
    ...
    public JSONObject pay(PayOrder payOrder) {
        ...
        switch (channelName) {
            case PayConstant.newTradeTypeConstant.TRADE_TYPE_ALIPAY:
                retObj = doQifuRyPayReq(payOrder, "31"); //支付宝H5
                break;
            default:
    ...
    public JSONObject doQifuRyPayReq(PayOrder payOrder, String pay_code) {
        String logPrefix = "【启富支付统一下单】";
        ...
        try {
            ...
            map.put("version","2.0.3");// 版本号
            map.put("merchant_no", channelPayConfig.getMchId());//商户号
            map.put("txn_type","create.order");//交易类型
            ...
            map.put("sign_method","MD5");//签名方法
            map.put("pay_type",pay_code);//支付类型
//            map.put("signature",channelPayConfig.getmD5Key());//签名
            System.out.println("下单验签串生成"+map.toString());
            String signature = PayDigestUtil.getSign(map, channelPayConfig.getmD5Key());
            _log.info(logPrefix + "下单验签串sign:{}", signature);
//            map.put("bank_code", pay_code);//支付方式
//            map.put("return_url",payConfig.getReturnUrl(channelName));//同步跳转地址
//            map.put("return_param","您好");//自定义参数
    ...
    public JSONObject query(PayOrder payOrder) {
        ...
        try {
            ...
            if (code.equals("0000")) {
                ...
                System.out.println("启富------》》》》》》《《《《《《++++++" + map1.toString());
