...
import org.xxpay.pay.mq.Mq4MchTransNotify;
 ...
 */
@Component
public class BaseNotify5MchTrans {

	private static final MyLog _log = MyLog.getLog(BaseNotify5MchTrans.class);
	...
	public String createNotifyUrl(TransOrder transOrder) {
		...
		if(mchInfo != null) {
			...
			if(agentInfo != null) key = agentInfo.getPrivateKey();
		}
		if(key == null) {
	...
	public void doNotify(TransOrder transOrder, boolean isFirst) {
		...
		try {
			mq5MchTransNotify.Send(object.toJSONString());
		} catch (Exception e) {
	...
	public JSONObject createNotifyInfo(TransOrder transOrder, boolean isFirst) {
		// 查询最新订单数据
		transOrder = rpcCommonService.rpcTransOrderService.findByTransOrderId(transOrder.getTransOrderId());
		String url = createNotifyUrl(transOrder);
		if(isFirst) {
			int result = rpcCommonService.rpcMchNotifyService.insertSelectiveOnDuplicateKeyUpdate(transOrder.getTransOrderId(), transOrder.getMchId(), transOrder.getAppId(), transOrder.getMchTransNo(), PayConstant.MCH_NOTIFY_TYPE_TRANS, url);
