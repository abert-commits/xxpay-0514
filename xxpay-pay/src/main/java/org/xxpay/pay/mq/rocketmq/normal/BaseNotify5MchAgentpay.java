 ...
 */
@Component
public class BaseNotify5MchAgentpay {

	private static final MyLog _log = MyLog.getLog(BaseNotify5MchAgentpay.class);
	 ...
	 * @return
	 */
	public String createNotifyUrl(MchAgentpayRecord mchAgentpayRecord) {
	 ...
	 * 处理商户代付通知
	 */
	public void doNotify(MchAgentpayRecord mchAgentpayRecord, boolean isFirst) {
		...
		try {
			mq5MchAgentpayNotify.Send(object.toJSONString());
		} catch (Exception e) {
	...
	public JSONObject createNotifyInfo(MchAgentpayRecord mchAgentpayRecord, boolean isFirst) {
		...
		if(isFirst) {
			int result = rpcCommonService.rpcMchNotifyService.insertSelectiveOnDuplicateKeyUpdate(mchAgentpayRecord.getAgentpayOrderId(),
					mchAgentpayRecord.getMchId(), null, mchAgentpayRecord.getMchOrderNo(), PayConstant.MCH_NOTIFY_TYPE_AGENTPAY, url);
