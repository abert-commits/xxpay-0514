 ...
 */
@Configuration
public class MySeq implements EnvironmentAware {
	...
	public static String getPayHmsSeq() {
		SimpleDateFormat fm = new SimpleDateFormat(DateUtil.FORMAT_HHMMSSSSS);
		return fm.format(new Date()) + pay_seq.getAndIncrement() % 10000 ;
	}
	...
	public static String getPdd() {
		return getSeq(check_seq_prefix, sett_seq);
	}
	...
	public static String getSeqString() {
		SimpleDateFormat fm = new SimpleDateFormat(DateUtil.FORMAT_YYYYMMDDHHMMSSSSS);
	...
	public void setEnvironment(Environment environment) {
		node = environment.getProperty("HX_NODE");
	}
}