        ...
        if(CollectionUtils.isNotEmpty(sysResourceList)) {
            for(SysResource sysResource : sysResourceList) {
                ...
                for(String url : permUrls) {
                    ...
                    //modify by terrfly 20181222 ，解决array只存在一个的问题，应为url对应多个权限ID
                    array = map.get(Constant.MGR_CONTROLLER_ROOT_PATH + url);
                    if(array == null){
                        array = new ArrayList<>();
                    }

                    array.add(cfg);
                    map.put(Constant.MGR_CONTROLLER_ROOT_PATH + url, array);
                }
            }
        }
    }
