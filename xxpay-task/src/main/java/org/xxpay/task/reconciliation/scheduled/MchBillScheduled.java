...
public class MchBillScheduled {
    ...
    public void buildMchBillTask() {
        ...
        Map<String, MchBill> handleMap = new HashedMap();
        ...
        while (flag) {
            ...
            for(PayOrder payOrder : payOrderList) {
                ...
                if(lastMchId == null || lastMchId.longValue() != mchId.longValue()) {
                    ...
                    if(csvWriter != null) {
                        csvWriter.close();
                        rpcCommonService.rpcMchBillService.updateComplete(lastMchId, billDate);
                        _log.info("商户:{},对账文件生成完成", lastMchId);
                    }

                    // 判断该商户是否已经生成过
                    MchBill mchBill = rpcCommonService.rpcMchBillService.findByMchIdAndBillDate(mchId, billDate);
                    if(mchBill == null) {
                        MchInfo mchInfo = rpcCommonService.rpcMchInfoService.findByMchId(mchId);
                        mchBill = new MchBill();
                        mchBill.setMchId(mchId);
                        mchBill.setMchName(mchInfo.getName());
                    ...
                    if(handleMap.get(mchId + billDateStr) == null) {
                        String scvFilePath = mchBillPath + billDateStr + File.separator + payOrder.getMchId() + ".csv";
                ...
                if(handleMap.get(mchId + billDateStr) == null) {
                    try {
                        String[] csvContent = { payOrder.getPayOrderId(), payOrder.getMchOrderNo(), payProductMap.get(payOrder.getProductId()+""), AmountUtil.convertCent2Dollar(StrUtil.toString(payOrder.getAmount())),
                                payOrder.getSubject(), payOrder.getBody(), payOrder.getChannelUser(), payOrder.getChannelOrderNo(), StrUtil.toString(payOrder.getMchRate()) + "%" , AmountUtil.convertCent2Dollar(StrUtil.toString(payOrder.getMchIncome())),
